...
class Image:
    def __init__(self, bitmap: Tensor, fname: str, orig_shape=None):
        self.bitmap     = bitmap
        self.fname      = fname
    ...
    def to_image_coord(self, xys: Tensor) -> Tensor:
    ...
    def _interpolate(self, image: Tensor, shape) -> Tensor:
    ...
    def _pad(self, image: Tensor, shape) -> Tensor:
...
def extract(dataset, save_path):
    ...
    for bitmaps, images in pbar:
        ...
        with torch.inference_mode():
            feature_list = model(bitmaps, n=args.n)

        for features, image in zip(feature_list, images):
            features = features.to(CPU)

            kps_crop_space = features.keypoints.T
            kps_img_space, mask = image.to_image_coord(kps_crop_space)

            keypoints   = kps_img_space.numpy().T[mask]
            descriptors = features.descriptors.numpy()[mask]
            scores      = features.detection_scores.numpy()[mask]
...
if __name__ == '__main__':
    ...
    model = DISK.from_pretrained('depth', device=DEV)
